...
/// The "back page" — sits behind the day timeline card.
/// Shows 7 day columns with mini task pill icons and gradient-colored vertical lines.
struct WeekOverviewView: View {
    ...
    /// Monday–Sunday of the week containing selectedDate
    ...
    private func dayColumn(for date: Date) -> some View {
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .top) {
                ...
                // ── FIX #2: BASELINE LINE — always visible, even with no tasks ──
                Path { p in
                    p.move(to: CGPoint(x: centerX, y: 0))
                    p.addLine(to: CGPoint(x: centerX, y: gridHeight))
                }
                .stroke(
                    Color.gray.opacity(isSelected ? 0.2 : 0.1),
                    lineWidth: 1
                )
                ...
                if dayTasks.count >= 2 {
                    ForEach(0..<dayTasks.count - 1, id: \.self) { i in
                        ...
                        let y1 = yFor(task.startTime) + pillHeight(for: task)
                ...
                // Solid lines through each task's duration
                ForEach(dayTasks) { task in
                    let y = yFor(task.startTime)
                    let h = pillHeight(for: task)
                ...
                // Mini pill icons at each task position
                ForEach(dayTasks) { task in
                    let y = yFor(task.startTime)
                    let h = pillHeight(for: task)
    ...
    private func pillHeight(for task: TaskItem) -> CGFloat {
